...
vector<meld> HandEvaluator::canDeclareBigMeldedKang(vector<Tile*>hand,
	Tile *discardedTile)
{
	vector<meld> kang;
	vector<vector<int>> split_melds = splitTiles(hand, &Tile::areEqual);
...
bool HandEvaluator::canDeclareMahjong(std::vector<Tile*> hand)
{
...
void HandEvaluator::test()
{
	...
	sort(tiles.begin(), tiles.end(), SortTiles<Tile*>());
	...
	sort(hand.begin(), hand.end(), SortTiles<Tile*>());
	...
	assert(canDeclareMeldedPeng(melds, tiles[0]).empty());
	cout << "Passed." << endl;;
