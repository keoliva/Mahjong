...
void Draw::drawGame(float rot_x, float rot_y, float rot_z, mouseActivity mouseInfo, Game *game)
{
    ...
    // regex re ("In_Play<(.+), (.+)>");
    char playerWind[6], playerMeld[20]; // longest wind has five characters
    ...
    for (int i = 0; i < NUM_PLAYERS; i++) {
        ...
        // draw revealed melds and bonus tiles
        {
            ...
            int j = firstIndex;
            for (pair<MeldType, player_tiles_t> meld : player->melds) {
            ...
            pieceIndex = 0;
            for (; j < lastIndex; j++) {
                revealed_tile = player->bonuses[pieceIndex++].get();
                loc = getPieceLoc(REVEALED, playerPos, j);
                tile.draw(loc.x, loc.y, loc.z, loc.rotX, loc.rotY, loc.rotZ,
        ...
        // draw discarded tiles
        {
            ...
            for (; pieceIndex < discardsSize; pieceIndex++) {
                ...
                _tile = (player->discards)[pieceIndex].get();
    ...
    if (game->getStatus() == In_Play(WAITING_FOR_INPUT_AFTER_DECLARATION)) {
		...
		drawGameOver(game->getStatus());
		displayChoices(game->getCurrentPlayer()); // displays winning hand
	}
    updateText();
}

void Draw::drawGameOver(Status game_status) {
	string status = game_status.toString();
	Status *curr_status = &game_status;
	Over *over_status = dynamic_cast<Over*>(curr_status);
	Outcome *outcome = over_status->outcome;
	stringstream ss;
	if (typeid(*outcome).name() == "Winner") {
		char playerWind[6], mahjongType[15];
		assert(sscanf(status.c_str(), "Over<Winner<%[A-Z]%*[, ]%[A-Z]>>", 
													playerWind, mahjongType) == 2);
		ss << playerWind << " declared 'MAHJONG' and won by '" << mahjongType << "'" << endl;
		ss << "\n Press 'r' to start a new game. Everyone will change their seats.";
	} else { // outcome is of type GameDraw
		ss << "This game is a DRAW. \n Press 'r' to start a new game. Everyone will keep their seats";
	}
	updates["gameOverMessage"] = msg_data(ss.str(), 4.0 / 6.0);
}
...
void Draw::displayChoices(Player *player, Tile *discardedTile) {
    map<MeldType, vector<meld>> options = player->getOptions();
    Declaration dec_type = player->getDeclaration().first;
        ...
        switch (dec_type) {
            ...
            case Declaration::MELDED_KANG: {
                if (dec_type == Declaration::SMALL_MELDED_KANG) {
                    tile_in_meld = player->hand[_meld.indexInHand].get();
                } else {
                    tile_in_meld = player->hand[_meld.indicesInHand[0]].get();
                }
            ...
            case Declaration::MELDED_CHI:
                ...
                for (int i = 0; i < num_tiles; i++) {
                    ...
                    if (_meld.indicesInHand[i] == -1) {
                        ...
                        tile_in_meld = player->hand[_meld.indicesInHand[i]].get();
                    }
                    tile.draw(loc.x, loc.y, z, loc.rotX, loc.rotY, loc.rotZ,
                              tile_in_meld->get_val());
                }
